...
- **오늘 목표:** 구현 착수 순서 1~10번 — 셋업 · 데이터 모델 · 정적 데이터 · 인증 · 온보딩/등록 흐름 · 핵심 가치 화면
...
- **진행한 작업:**
  ...
  - **SC-03 오늘 탭 정보 밀도 개편**(피드백) — 메인 탭인데 담긴 정보가 적다는 지적. 헤더에 오늘 날짜 추가, 날씨 카드를 확장(습도·강수 지표 분할 + `lib/weather-tips.ts`의 날씨 상태별 일반 팁), 식물 선택 가로 칩(`components/PlantSelector.tsx`) 신설, 선택에 따라 하단이 전환되도록 재구성 — 전체는 오늘의 할일 + 다가오는 일정(신규), 식물별은 물주기·분갈이·배치 상태 카드(`components/CareStatCard.tsx`)와 종별 케어팁. `lib/care-service.ts`에 `loadUpcomingCareItems` 추가
  - **10번 제품 탭·케어 리포트** — SC-09(카테고리 필터 칩 · 2열 제품 그리드 · 품절 배지와 선택 비활성화 · 분갈이 시기 30일 이내면 화분·흙 추천 섹션 상단 고정) / SC-10(제품 정보·가격·리뷰 요약 · 제휴 링크 새 탭 이동, 품절이면 버튼 비활성화) / SC-11 케어 리포트 카드(주간 완료율·연속 관리일수). SC-07 분갈이 탭에도 시기가 다가오면 제품 탭으로 보내는 배너 추가(화면 흐름도의 SC-07 → SC-09 tab전환)
  - **9번 마이페이지** — SC-11(프로필 인라인 수정 · 위치 재설정 · 알림 on/off 토글 · 권한 배지 2종 · 계정 관리 진입) / SC-12(로그아웃 · 회원탈퇴 확인 모달). 위치 요청 로직은 SC-02와 중복이라 `lib/location.ts`로 분리해 양쪽에서 재사용. 케어 리포트 카드는 Phase 3라 10번으로 미룸
  - SC-06 우측 정보를 두 개의 분리된 블록으로 변경(피드백) — 마지막 물준 날짜는 그레이(덜 중요), 다음 예정일은 다크, 물 줄 당일부터 라임
  ...
  - SC-06 카드 구조 변경(피드백) — 카드마다 배경색이 달라 산만하다는 지적에 따라 카드는 흰색으로 통일하고, 우측에 물주기 정보 블록을 분리 배치. 블록에는 마지막 물준 날짜와 다음 예정일을 위아래로 표시하고(D-day 표기에서 실제 날짜 표기로 재차 변경), 블록 색으로 급함을 구분(오늘·지난 항목은 라임, 그 외 블랙). 카드 본문에는 배치 위치·화분 크기를 노출
...
- **스펙과 다르게 구현한 부분 + 이유:**
  ...
  - 회원탈퇴에서 계정(`auth.users`) 자체 삭제는 미구현 — service_role 키가 필요한데 클라이언트에 둘 수 없고 별도 서버 함수가 필요함. 현재는 등록 식물을 '삭제' 상태로 바꾸고 로그아웃까지만 수행. 정책상 하위 데이터 30일 유예 파기도 스케줄러가 필요해 MVP 범위 밖
  - 식물별 알림 on/off는 구현하지 않음 — 데이터정의서의 알림 권한은 사용자 단위 필드 하나뿐이라 식물별 설정은 새 필드가 필요, 사용자 판단으로 보류
  - SC-03에 화면설계서 구성요소에 없는 요소 3개 추가 — 식물 선택 가로 탭 · 다가오는 일정 · 날씨 상태별 팁. 메인 탭 정보량이 부족하다는 사용자 요청에 따른 것으로, 날씨 팁 문구도 기획서에 없어 새로 작성(`lib/weather-tips.ts` 한 곳에 모아 교체 가능하게 둠). 확정 필요
  - 케어 리포트 지표 정의가 문서에 없어 임의 정의 — 주간 완료율은 최근 7일 예정 항목 중 완료 비율, 연속 관리일수는 완료 기록이 끊기지 않은 일수(오늘 아직 안 했어도 어제까지 이어졌으면 유지). 확정 필요
  - SC-09의 "카탈로그 조회 실패" 에러 분기는 미구현 — 제품이 코드 내 JSON이라 조회 실패가 발생할 수 없음
