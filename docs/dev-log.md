...
- **오늘 목표:** 구현 착수 순서 1~8번 — 셋업 · 데이터 모델 · 정적 데이터 · 인증 · 온보딩/등록 흐름 · 핵심 가치 화면
...
- **진행한 작업:**
  ...
  - **8번 캘린더** — SC-05 월간/주간 캘린더. `lib/calendar.ts`에 날짜 격자 생성·범위 이동·라벨 포맷을 분리하고, 케어 유형별 배지 색(물주기 라임 · 분갈이 라벤더 · 완료는 흐린 회색), 날짜 선택 시 하단 리스트 갱신, 리스트에서 완료 체크 시 SC-03과 같은 처리 재사용
  - **7번 분갈이·위치 확장** — SC-07을 탭형 3섹션(물주기 / 분갈이 / 배치 위치)으로 재구성. 분갈이 탭은 다음 권장 시기 D-day와 이력, 화분 크기 변경(바꾸면 분갈이 시기 재계산 + 미완료 예정 이력까지 갱신). 배치 위치 탭은 인라인 수정, 종별 권장 광량과 맞으면 라임·다르면 라벤더 카드로 구분. 분갈이 완료 처리도 추가 — 다음 분갈이는 입양일이 아니라 이번 분갈이 날짜 기준으로 다시 계산
  - **컬러 정체성 보강** — 흰/회색 비중이 높아 팔레트가 드러나지 않는다는 피드백에 따라 참고 이미지(typo_03)의 리스트 패턴을 도입. `lib/tone.ts`에 라임 → 블랙 → 흰 순으로 카드 색을 돌리는 헬퍼를 두고 SC-03 할일 카드·SC-06 식물 카드에 적용, SC-04 빈 상태와 SC-03 완료 상태도 라임 블록으로 교체, SC-07 프로필은 블랙 카드에 라임 D-day 배지
...
- **막힌 점 / 트러블슈팅:**
  ...
  - 캘린더에서 주간 뷰로 전환하면 선택 날짜가 보이는 범위 밖으로 나가 하단 리스트가 엉뚱하게 비어 보임 → 범위가 바뀔 때 선택 날짜가 화면 밖이면 오늘(또는 범위 첫날)로 되돌리도록 보정
  - 선택 날짜와 오늘이 같은 스타일이라 오늘이 어딘지 알 수 없었음 → 오늘 칸에 라임 링 추가
...
- **스펙과 다르게 구현한 부분 + 이유:**
  ...
  - 캘린더의 "날짜별 날씨 아이콘"은 오늘 칸에만 표시 — 무료 티어에 과거·미래 일별 날씨가 없음(유료 History API 필요). 유료 API 금지가 제약이라 범위를 줄임
  - "날짜별 식물 배지(색상 구분)"의 구분 기준이 화면설계서에 없어 케어 유형(물주기/분갈이)으로 구분. 확정 필요
...
- **내일 할 일:**
  ...
  - 9번 마이페이지 — SC-11 홈(알림 설정 on/off UX) · SC-12 계정 관리
